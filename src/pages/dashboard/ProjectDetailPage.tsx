...
export default function ProjectDetailPage() {
  ...
  // Scripts is the client's last approval gate before the machine takes over
  const scriptsRow = stagesByKey.get('scripts')
  const scriptsApprovedAt = (scriptsRow?.content?.approved_at as string | undefined) ?? null
  const scriptsApprovedBy = (scriptsRow?.content?.approved_by as string | undefined) ?? null
  const signedOff = currentWorkflow.stage === 'scripts' && !!scriptsApprovedAt
  const needsSignOff = currentWorkflow.stage === 'scripts' && !scriptsApprovedAt
  ...
  const handleSaveEdit = async () => {
    ...
    setSaving(true)
    try {
      // editing the copy invalidates any sign-off — the approval was for the
      // text as it read then. (An AI re-run replaces content wholesale, so
      // that path drops it on its own.)
      const { approved_at: _a, approved_by: _b, ...keep } = currentStageRow.content ?? {}
      await updateStage(project.id, currentWorkflow.stage, {
        content: { ...keep, text: editingText },
      })
      setEditingText(null)
      reload()
    } catch (err) {
  ...
  /**
   * Scripts are the last thing a client reads before the machine takes over,
   * so they get an explicit sign-off recorded on the stage (who + when)
   * rather than a silent "Next Stage".
   */
  const handleSignOff = async () => {
    ...
    try {
      await updateStage(project.id, currentWorkflow.stage, {
        content: {
          ...(currentStageRow.content ?? {}),
          approved_at: new Date().toISOString(),
          approved_by: user?.name || user?.email || 'the agency',
        },
      })
      await handleNextStage()
    } catch (err) {
      setNotice(err instanceof Error ? err.message : 'Could not sign off this stage.')
    } finally {
      setSaving(false)
    }
  }

  /** Jump the view to a stage by key (Shoot Plan → back to Scripts). */
          ...
          {workflowStages.map((stage, i) => {
            ...
            return (
              ...
              >
                ...
                {stage.stage === 'scripts' && scriptsApprovedAt && (
          ...
          <div className="bg-brand-900/20 border border-white/5 rounded-xl overflow-hidden">
            ...
            <div className="p-6">
              {editingText !== null ? (
                ...
                <>
                {currentWorkflow.stage === 'shootplan' && currentStatus === 'completed' && (
                  <div className="mb-5 px-4 py-3 bg-brand-900/40 border border-white/[0.06] rounded-lg flex items-start gap-2.5">
                    ...
                    <p className="text-brand-400 text-xs font-body leading-relaxed">
                      Read-only summary of how the signed-off scripts will be shot — there&apos;s nothing to approve
                      here. If something looks wrong, it comes from the scripts:{' '}
                      <button onClick={() => goToStage('scripts')} className="text-white underline underline-offset-2 hover:text-brand-200 transition-colors">
                        go back to Scripts
                      </button>{' '}
                      and re-run this stage after.
                    </p>
                  </div>
                )}
                {signedOff && (
                  <div className="mb-5 px-4 py-3 bg-green-500/[0.06] border border-green-400/15 rounded-lg flex items-center gap-2.5">
                    <svg className="w-4 h-4 text-green-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                    </svg>
                    <p className="text-green-300/90 text-xs font-body">
                      Signed off by {scriptsApprovedBy ?? 'the agency'}
                      {scriptsApprovedAt ? ` · ${timeAgo(scriptsApprovedAt)}` : ''}. Editing or re-running the scripts clears this.
                    </p>
                  </div>
                )}
                <StageBody
                ...
                {isRunning && editingText === null ? (
                  ...
                  <>
                    {currentWorkflow.stage === 'discovery' ? (
                      ...
                      <>
                        {currentWorkflow.stage === 'shootplan' ? (
                          /* The Shoot Plan is a machine-facing summary of the
                             signed-off scripts — nothing here is copy to edit.
                             Disagreements belong back in Scripts. */
                          <button
                            onClick={() => goToStage('scripts')}
                            className="px-4 py-2.5 border border-white/15 text-white font-heading text-sm rounded-lg hover:border-white/30 hover:bg-white/[0.03] transition-all flex items-center gap-2"
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {signedOff && (
                      <span
                        title={`Signed off by ${scriptsApprovedBy ?? 'the agency'}${scriptsApprovedAt ? ` · ${timeAgo(scriptsApprovedAt)}` : ''}`}
                        className="px-3 py-1.5 rounded-lg bg-green-500/10 border border-green-400/20 text-green-400 text-[10px] font-heading font-bold tracking-wider flex items-center gap-1.5"
                      >
                    ...
                    {STAGE_ORDER.indexOf(project.current_stage) === STAGE_ORDER.indexOf(currentWorkflow.stage) &&
                      ...
                      needsSignOff ? (
                        <button
                          ...
                          disabled={saving}
                          className="px-5 py-2.5 bg-white text-black font-heading font-bold text-sm rounded-lg hover:bg-brand-200 transition-all flex items-center gap-2 disabled:opacity-40"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                          </svg>
                          {saving ? 'Signing off…' : 'Sign Off Scripts'}
                        </button>
                      ) : (
                        <button
